...
sperm whale codas" — Project CETI (Nature, 2025)

Original MATLAB code: https://github.com/Project-CETI/Coda-detector
Python port by Claude, with Kevin — 3 mars 2026.
"""
...
MAT_DIR = Path(__file__).parent.parent / "Coda-detector" / "Functions"
...
def teager_kaiser(sig):
    """
    Teager-Kaiser Energy Operator.
    Psi[x[n]] = x^2[n] - x[n-1] * x[n+1]
    Enhances impulsive transients like whale clicks.
    """
...
def resample_to_target(data, fs_orig, fs_target):
...
def detect_clicks_tkeo(filtered_signal, fs, params):
    """
...
def select_transients(filtered_signal, click_candidates, fs, params):
    """
    Filter click candidates by SNR threshold.
    Keep only the strongest transients.
    """
    clicks = []
...
def estimate_ipi(filtered_signal, click_sample, fs, params):
    ...
    peaks, props = signal.find_peaks(tkeo_norm, distance=min_dist)
...
def extract_waveforms(filtered_signal, clicks, fs, seg_ms=16.0, asymmetry=0.12):
...
def compute_similarity_matrix(clicks, params):
    """
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if amp_max > 0:
                amp_sim = 1.0 - abs(clicks[i].amplitude - clicks[j].amplitude) / amp_max
            else:
                amp_sim = 0.0
...
def cluster_clicks_into_codas(clicks, sim_matrix, params):
    """
...
def remove_duplicate_codas(codas, params):
    ...
    for i in range(len(codas)):
        ...
        for j in range(i + 1, len(codas)):
            ...
            if not icis_i:
                continue
            matches = sum(
                1 for a, b in zip(icis_i, icis_j)
...
def detect_codas(audio_path, params=None, progress_callback=None):
    """
    ...
    for buf_idx in range(n_buffers):
        ...
        buffer = filtered[start:end]
        time_offset = start / fs

        if progress_callback:
            progress_callback(buf_idx + 1, n_buffers, time_offset)

        click_candidates = detect_clicks_tkeo(buffer, fs, params)
        if not click_candidates:
            continue

        clicks = select_transients(buffer, click_candidates, fs, params)
        if len(clicks) < params.min_coda_clicks:
            continue

        for click in clicks:
            sample = int(click.time * fs)
            click.ipi = estimate_ipi(buffer, sample, fs, params)
...
def codas_to_dict(codas):
    """Convert codas to a list of dicts for easy export."""
    results = []
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python coda_detector.py <audio_file.wav>")
        sys.exit(1)

    audio_path = sys.argv[1]
    print(f"Detecting codas in: {audio_path}")

    def progress(buf, total, time):
        print(f"  Buffer {buf}/{total} (t={time:.1f}s)")

    codas = detect_codas(audio_path, progress_callback=progress)
    results = codas_to_dict(codas)

    print(f"\nDetected {len(codas)} codas:")
    for r in results:
        icis_str = ", ".join(f"{ici*1000:.0f}ms" for ici in r['icis'])
        print(f"  Coda {r['coda_id']}: {r['n_clicks']} clicks at t={r['start_time']:.2f}s "
              f"(duration={r['duration']*1000:.0f}ms, ICIs=[{icis_str}])")

    output_path = Path(audio_path).stem + "_codas.json"
